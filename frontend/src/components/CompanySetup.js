                ...
                {sisterCompanyAccountingSystem && sisterCompanyForm.country_code && (
                  <div className="md:col-span-2">
                    <Alert className="w-full">
                      ...
                      <AlertDescription className="w-full alert-wrap">
                        <div className="space-y-1">
                          <div><strong>Accounting System:</strong> <span className="currency-no-truncate">{sisterCompanyAccountingSystem.name}</span></div>
                          <div><strong>Base Currency:</strong> {sisterCompanyForm.base_currency}</div>
                          <div><strong>Fiscal Year:</strong> Starts {sisterCompanyAccountingSystem.fiscal_year_start}</div>
                        </div>
                      </AlertDescription>
                    </Alert>
                  </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <Select value={formData.base_currency} onValueChange={(value) => setFormData(prev => ({...prev, base_currency: value}))}>
          <SelectTrigger tabIndex={1} className="w-full min-h-[3rem]" style={{overflow: 'visible'}}>
            <SelectValue placeholder="Select base currency" className="currency-no-truncate">
              {formData.base_currency ? (
                <span className="font-medium text-gray-900 currency-no-truncate">
                  {formData.base_currency} {currencies.find(c => c.code === formData.base_currency)?.symbol} - {currencies.find(c => c.code === formData.base_currency)?.name}
                </span>
              ) : (
          ...
          <SelectContent className="min-w-[600px] w-auto max-h-[300px] overflow-y-auto">
            {currencies.map(currency => (
              <SelectItem key={currency.code} value={currency.code} className="py-3">
                <span className="text-base font-medium text-gray-900 currency-no-truncate">
      ...
      <div>
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto border rounded-lg p-4 bg-gray-50">
          {currencies.filter(c => c.code !== formData.base_currency).map(currency => (
            <div 
              key={currency.code} 
              className={`w-full p-3 border rounded-lg cursor-pointer transition-all no-truncate ${
            ...
            >
              <div className="flex items-center justify-between no-truncate">
                <div className="flex items-center space-x-3 no-truncate">
                  <span className="text-lg">{currency.symbol}</span>
                  <div className="flex flex-col">
                    <span className="text-sm font-semibold text-gray-900 no-truncate">{currency.code}</span>
                    <span className="text-xs text-gray-600 no-truncate">{currency.name}</span>
                  </div>
                </div>
                {formData.additional_currencies.includes(currency.code) && (
